...
package com.gotak.address.layers;
...
public class LayerDownloadExample extends BroadcastReceiver
    ...
    private static final String TOOL_FINISH = "com.gotak.address.layers.LayerDownload_TOOL_FINISH";
    ...
    public LayerDownloadExample(MapView mapView, Context plugin) {
        ...
        _downloader = new LayerDownloader(mapView.getContext());
    ...
    private void downloadLayer(Shape shape, double distM) {
        ...
        rb.setTitle(title);
        rb.setSourceURI(sourceURI);
        rb.setResolution(minRes, maxRes);
        rb.setShape(shape);
        rb.setExpandDistance(distM);

        // Set the event callback receiver to this class, which implements
        // the LayerDownloader.Callback
        _downloader.setCallback("layerdownloadexample", this);

        // Start the download service
        _downloader.startDownload(rb);
    ...
    @Override 
    public void onRegionSelectFinished(Intent i) { }
}
